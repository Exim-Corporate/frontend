...
import type { StrapiHomePage, StrapiSingleResponse } from '../../types/strapi';
...
export default defineEventHandler(async event => {
  ...
  const fetchPage = async (requestedLocale?: string) =>
    await $fetch<StrapiSingleResponse<StrapiHomePage>>(
      `${strapiUrl}/api/home-page?${stringify(
        {
  ...
  try {
    ...
    console.error('[home-page] Failed to fetch page content from Strapi', {
      locale,
      strapiUrl,
